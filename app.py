...
# ==========================================
# ENRUTAMIENTO (LÓGICA DEL QR VS SISTEMA)
# ==========================================
query_params = st.query_params

# 1. SI SE ESCANEA UN CÓDIGO QR (Muestra SOLO las fotos)
if "pod_id" in query_params:
    target_pod = query_params["pod_id"]
    st.title(f"📸 Evidencias Fotográficas - POD #{target_pod}")
    ...
    try:
        ...
        if len(df_fotos) > 0:
            ...
            st.success("✅ Este POD se recibió sin reportes de cajas dañadas.")
    except Exception as e:
        st.error(f"Error al cargar las fotos: {e}")
        
    st.markdown("---")
    if st.button("⬅️ Ir al Sistema Principal"):
        st.query_params.clear()
        st.rerun()

# 2. SI SE ENTRA NORMALMENTE (Muestra el sistema completo)
else:
    # --- MENÚ LATERAL ---
